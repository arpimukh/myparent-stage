...
const fetchVendorDetails = async (userId) => {
  ...
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || '/v1'}/api/vendor-details/${userId}`, {
      headers: {
    ...
    if (result.success) {
     ...
     // if (typeof services === 'string') {
        ...
        console.log('   - Final   array:', services)
  ...
  const handleBackToMain = () => {
    ...
    setSearchFilters({ clientName: '', areaPin: '' })
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case 'Accept': return '#4FC3F7'
      case 'Reject': return '#f0cf62'
      case 'Close': return '#4b4a48'
      default: return '#b1117c'
    }
  }
  ...
  return (
    <>
      <Head>
        <title>Vendor Dashboard - Parent Care Services</title>
      </Head>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                  ...
                  <div>
                    ...
                    <div style={{ marginBottom: '15px' }}>
                      ...
                      <div style={{ fontSize: '16px', fontWeight: 'bold', color: '#2d3748', marginTop: '5px' }}>
                        {loggedInVendor.service_description}
                      </div>
                    </div>
      ...
      {/* {//kafka websocket  component}
      // components/VendorLiveFeed.js
        socket.on('new_job_available', (job) => {
          setJobs(prev => [...prev, job]);
        });

        socket.on('job_updates', ({ jobId, status }) => {
          if (status === 'assigned') {
