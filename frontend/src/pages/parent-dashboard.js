...
/>  */}
          ...
          <div style={{ background: 'white', borderRadius: '15px', padding: '30px', marginBottom: '30px', boxShadow: '0 4px 15px rgba(0,0,0,0.1)' }}>
            ...
            <div style={{
              display: 'grid',
              gridTemplateColumns: '1fr 2fr 1fr 1fr 1fr  1fr 1fr 1.1fr 1.1fr',
              background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',
              color: 'white',
              padding: '5px',
              borderRadius: '10px 10px 0 0',
    ...
    <div>SERVICE STATUS</div> */}
            ...
            {isLoadingServices && closedServices.map((service, index) => (
              <div key={index} style={{
                display: 'grid',
                gridTemplateColumns: '1fr 2fr 1fr 1fr 1fr  1fr 1fr 1fr 1.2fr',
                padding: '5px',
                borderBottom: '1px solid #f0f0f0',
                alignItems: 'center',
                background: index % 2 === 0 ? '#fafafa' : 'white',
...
/>  */}
                ...
                <div className="flex justify-center gap-2 py-2">
                     {[1, 2, 3, 4, 5].map((star) => (
                          <button
                            key={`${service.id}-${star}`}
                            onMouseEnter={() => setHoverState(prev => ({ ...prev, [service.id]: star }))}
                            onMouseLeave={() => setHoverState(prev => ({ ...prev, [service.id]: 0 }))}
                            onClick={() => setRatingState(prev => ({ ...prev, [service.id]: star }))}
                            className="transition-transform active:scale-90"
                          >
                            <Star
                            size={16}
                            fill={(hoverState[service.id] || rating[service.id] || service.rating || 0) >= star ? "#f59e0b" : "none"}
                            className={(hoverState[service.id] || rating[service.id] || service.rating || 0) >= star ? "text-amber-500" : "text-slate-200"}
                            strokeWidth={2.5}
                            />
                          </button>
                          ))}
                ...
                <div>
                  <button
                    onClick={() => updateReview(index, rating[service.id] || 0)}
                    style={{
                      background: '#e2e8f0',
                      color: '#4a5568',
                      border: 'none',
